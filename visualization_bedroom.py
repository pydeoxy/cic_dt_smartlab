...
def visualize_real_time_data(db_path, json_path):
    ...
    # Bedroom-specific sensor topics
    bedroom_topics = [
        "KNX/14/0/0<Bedroom.Sensors.CO2-ppm>",  # CO2
        "KNX/14/0/2<Bedroom.Sensors.Air-temperature-C>",  # Air Temperature
        "KNX/14/0/1<Bedroom.Sensors.Rh-percent>"  # Relative Humidity
    ]

    # Set up the figure and subplots
    fig, axs = plt.subplots(len(bedroom_topics), 1, figsize=(10, 12))  # Subplots for each topic
    ...
    while running:
        for i, topic in enumerate(bedroom_topics):
