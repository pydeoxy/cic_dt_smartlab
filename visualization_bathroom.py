...
OCCUPANCY_THRESHOLD = 429.5  # CO2 value (ppm) above which the room is considered occupied
...
def visualize_real_time_data(db_path, json_path):
    ...
    # Bathroom-specific sensor topics
    bathroom_topics = [
        "KNX/15/0/0<Bathroom.Sensors.CO2-ppm>",  # CO2
        "KNX/15/0/2<Bathroom.Sensors.Air-temperature-C>",  # Air Temperature
        "KNX/15/0/1<Bathroom.Sensors.RH|percent>"  # Relative Humidity
    ]

    # Set up the figure and subplots
    fig, axs = plt.subplots(len(bathroom_topics), 1, figsize=(10, 12))  # Subplots for each topic
    ...
    while running:
        for i, topic in enumerate(bathroom_topics):
